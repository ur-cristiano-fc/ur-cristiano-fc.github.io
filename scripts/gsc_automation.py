# Save this as gsc_automation.py
...
class GSCAutomation:
    def __init__(self, profile_path="./chrome_profile", headless=False, record_video=True):
    ...
    def setup_driver(self):
        """Setup Chrome driver with optional headless mode"""
        options = Options()
        ...
        if self.headless:
            ...
            if self.record_video:
                print("⚠️ Video recording disabled in headless mode")
        ...
        if not self.headless:
            ...
            time.sleep(1)  # Wait for window to maximize before recording
    ...
    def start_recording(self, filename=None):
        ...
        if not self.record_video or self.headless:
            return
        
        if filename is None:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"gsc_automation_{timestamp}.mp4"
        
        self.video_path = self.recordings_folder / filename
        
        # Get screen dimensions from the browser window
        window_size = self.driver.get_window_size()
        window_pos = self.driver.get_window_position()
        
        self.screen_size = (window_size['width'], window_size['height'])
        self.window_pos = (window_pos['x'], window_pos['y'])
    ...
    def _record_screen(self):
        """Internal method to record screen in background"""
        while self.recording:
            try:
                # Capture the browser window area
                x, y = self.window_pos
...
if __name__ == "__main__":
    # Configuration
    YOUR_DOMAIN = "https://langchain-tutorials.github.io/"
    USE_DOMAIN_PROPERTY = False
    
    # Define URLs to submit
    URLS_TO_SUBMIT = [
        "https://langchain-tutorials.github.io/langchain-production-deployment-staging-to-live-48-hours/",
    ]
    
    # Initialize bot (set headless=True to run without GUI, record_video=False to disable recording)
    bot = GSCAutomation(headless=False, record_video=True)
